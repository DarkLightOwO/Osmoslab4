...
subplot(3, 1, 3);
hold on;
fprintf('                                                                   Corr x1, shift x1 in C\n');
fprintf(' С| B1| B2| B3| B4| B5| B6| B7| B8| B9|B10|B11|B12|B13|B14|B15|B16|B17|B18|B19|B20|B21|B22|B23|B24|B25|B26|B27|B28|B29|B30|B31|А.Kор\n');
randmass = vivodlast;
autocorr_values=zeros(36, 1);

vivodlast = randmass1;
randmassforcor = randmass1;
shifts = 1:36;
for k = 1:36
    fprintf('%2d', k);
    if k ~= 1
        lastvivod = randmassforcor(31);
        for n = 1:30
            randmassforcor(32-n) = randmassforcor(31-n);
        end
        randmassforcor(1) = lastvivod;
    end
    sumcorr = 0;
    sumuncorr = 0;
    for l = 1:31
        fprintf('|%3d', randmassforcor(l));
...
fprintf('корреляция между двумя последовательностями = ');
disp(r(32));